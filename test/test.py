...
async def test_start(dut):
  ...
  for i in range(8):
      ...
      assert dut.TXD.value == (uart_byte & 1)
      uart_byte >>= 1
  await Timer(bit_time, "ns")
  ...
  divider = 2
  for i in range(8):
      assert dut.spi_cs_n == 0
      assert dut.spi_clk == 0
  ...
  for i in range(10):
    ...
    await send_instr(dut, InstructionADDI(x1, x0, (sel << 7) ^ 0x80).encode())
